...
describe("STRATEGY CONTRACT DEPLOYMENT", function () {
  ...
  it("Should Deploy Strategy", async function () {
    ...
    const Strategy = await ethers.getContractFactory("contracts/uranus_strategy_old.sol:GammaStrategy_Uranus");
...
describe("TRANSFER WANT BALANCE INTO THE USER2 account",function(){
  ...
  it("should transfer and check the want balance of `want_token_holder` address to user2`",async function () {
    ...
    const want_token_holder_want_bal = (await want_inst.balanceOf(want_token_holder));
    ...
    if(want_token_holder_want_bal > 0){
      ...
      await want_inst.connect(signer).transfer(user2.address,want_token_holder_want_bal);
...
describe(`DEPOSIT ${depositAmount/BigInt(1e18)} WANT in new pool`,function(){
  ...
  it(`should deposit user2 ${depositAmount/BigInt(1e18)} WANT `,async function() {
  ...
  it(`should deposit user2 ${depositAmount/BigInt(1e18)} WANT AGAIN AFTER 1 hour`,async function() {
...
describe(`TEST WITHDRAW ${withdrawAmount/BigInt(1e18)} WANT TOKENS`,function() {
  ...
  it("should print user2 pending gamma after withdraw after 1 block",async function() {
    ...
    pending_gamma_in_old_strat = (await strategy.userPendingGammaProfit(user2.address)).pendingProfitInGamma
...
describe("STRATEGY CONTRACT DEPLOYMENT", function () {
  ...
  it("Should Deploy Strategy", async function () {
    ...
    [user1,user2] = await ethers.getSigners();
    owner = await ethers.getSigner(ownersAccount);
    const Strategy = await ethers.getContractFactory("contracts/uranus_strategy_new.sol:GammaStrategy_Uranus");
    strategy = await Strategy.deploy(
      params[0],
      params[1],
      params[2],
...
describe(`TEST WITHDRAW ${withdrawAmount/BigInt(1e18)} WANT TOKENS`,function() {
  ...
  it("should print user2 pending gamma after withdraw after 1 block",async function() {
    ...
    pending_gamma_in_new_strat = (await strategy.userPendingGammaProfit(user2.address)).pendingProfitInGamma
    //console.log("PENDING GAMMA AFTER MINING 1 BLOCK AFTER WITHDRAW",(await strategy.userPendingGammaProfit(user2.address)).pendingProfitInGamma);
  });
...
describe(`PRINT PENDING GAMMA RESULTS FROM BOTH OLD AND NEW STRATEGIES`,function() {
  it("PRINT",async function(){
    console.log("\n PENDING GAMMA FOR OLD STRATEGY : ",pending_gamma_in_old_strat);
    console.log("\n PENDING GAMMA FOR NEW STRATEGY : ",pending_gamma_in_new_strat);
  })
})
