...
contract Gamma {
    ...
    uint public constant totalSupply = 100000000e18; // 100 million Gamma
    ...
    function approve(address spender, uint rawAmount) external returns (bool) {
        ...
        if (rawAmount == uint(-1)) {
            ...
            amount = safe96(rawAmount, "Gamma::approve: amount exceeds 96 bits");
        }
    ...
    function transfer(address dst, uint rawAmount) external returns (bool) {
        uint96 amount = safe96(rawAmount, "Gamma::transfer: amount exceeds 96 bits");
    ...
    function transferFrom(address src, address dst, uint rawAmount) external returns (bool) {
        ...
        uint96 amount = safe96(rawAmount, "Gamma::approve: amount exceeds 96 bits");

        if (spender != src && spenderAllowance != uint96(-1)) {
            uint96 newAllowance = sub96(spenderAllowance, amount, "Gamma::transferFrom: transfer amount exceeds spender allowance");
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public {
        ...
        require(signatory != address(0), "Gamma::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "Gamma::delegateBySig: invalid nonce");
    ...
    function getPriorVotes(address account, uint blockNumber) public view returns (uint96) {
        require(blockNumber < block.number, "Gamma::getPriorVotes: not yet determined");
    ...
    function _transferTokens(address src, address dst, uint96 amount) internal {
        require(src != address(0), "Gamma::_transferTokens: cannot transfer from the zero address");
        require(dst != address(0), "Gamma::_transferTokens: cannot transfer to the zero address");

        balances[src] = sub96(balances[src], amount, "Gamma::_transferTokens: transfer amount exceeds balance");
        balances[dst] = add96(balances[dst], amount, "Gamma::_transferTokens: transfer amount overflows");
    ...
    function _moveDelegates(address srcRep, address dstRep, uint96 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, "Gamma::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, "Gamma::_moveVotes: vote amount overflows");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint96 oldVotes, uint96 newVotes) internal {
      uint32 blockNumber = safe32(block.number, "Gamma::_writeCheckpoint: block number exceeds 32 bits");
 ...
 * @dev Storage for the gammatroller is at this address, while execution is delegated to the `implementation`.
 * GTokens should reference this contract as their gammatroller.
 */
...
// File: lending/GammatrollerStorage.sol
...
contract GammatrollerV2Storage is GammatrollerV1Storage {
    struct Market {
...
contract GammatrollerV3Storage is GammatrollerV2Storage {
    ...
    /// @notice The rate at which the flywheel distributes GAMMA, per block
    uint public gammaRate;
    ...
    /// @notice The GAMMAborrow index for each market for each supplier as of the last time they accrued GAMMA
...
contract GammatrollerV4Storage is GammatrollerV3Storage {
...
contract GammatrollerV5Storage is GammatrollerV4Storage {
    /// @notice The portion of GAMMAthat each contributor receives per block
    mapping(address => uint) public gammaContributorSpeeds;

    /// @notice Last block at which a contributor's GAMMArewards have been allocated
    mapping(address => uint) public lastContributorBlock;
}

// File: lending/PriceOracle.sol
...
// File: lending/EIP20Interface.sol
...
// File: lending/ExponentialNoError.sol
...
// File: lending/ErrorReporter.sol
...
// File: lending/EIP20NonStandardInterface.sol
...
// File: lending/InterestRateModel.sol

pragma solidity ^0.5.16;

/**
  * @title Green Planet's InterestRateModel Interface
  * @author Green Planet
  */
...
// File: lending/GTokenInterfaces.sol
...
// File: lending/GammatrollerInterface.sol

pragma solidity ^0.5.16;

contract GammatrollerInterface {
...
/**
 * @title Green Planet's GToken Contract
...
contract GToken is GTokenInterface, Exponential, TokenErrorReporter {
     ...
     */
    function initialize(GammatrollerInterface gammatroller_,
    ...
    function mintFresh(address minter, uint mintAmount) internal returns (uint, uint) {
        ...
        if (allowed != 0) {
            return (failOpaque(Error.GAMMATROLLER_REJECTION, FailureInfo.MINT_GAMMATROLLER_REJECTION, allowed), 0);
        }
        ...
        // unused function
        // gammatroller.mintVerify(address(this), minter, vars.actualMintAmount, vars.mintTokens);
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint, uint) {
        ...
        if (allowed != 0) {
            return (failOpaque(Error.GAMMATROLLER_REJECTION, FailureInfo.REPAY_BORROW_GAMMATROLLER_REJECTION, allowed), 0);
        }
    ...
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, GTokenInterface gTokenCollateral) internal returns (uint, uint) {
        ...
        if (allowed != 0) {
            return (failOpaque(Error.GAMMATROLLER_REJECTION, FailureInfo.LIQUIDATE_GAMMATROLLER_REJECTION, allowed), 0);
        }
        ...
        require(amountSeizeError == uint(Error.NO_ERROR), "LIQUIDATE_GAMMATROLLER_CALCULATE_AMOUNT_SEIZE_FAILED");
        ...
        /* We call the defense hook */
        // unused function
        // gammatroller.liquidateBorrowVerify(address(this), address(gTokenCollateral), liquidator, borrower, actualRepayAmount, seizeTokens);
    ...
    function seizeInternal(address seizerToken, address liquidator, address borrower, uint seizeTokens) internal returns (uint) {
        ...
        /* We call the defense hook */
        // unused function
        // gammatroller.seizeVerify(address(this), seizerToken, liquidator, borrower, seizeTokens);
...
/**
 * @title Green Planet's Gammatroller Contract
 * @author Green Planet
 */
contract Gammatroller is GammatrollerV5Storage, GammatrollerInterface, GammatrollerErrorReporter, ExponentialNoError {
    ...
    event GammaSpeedUpdated(GToken indexed gToken, uint newSpeed);
    ...
    event DistributedSupplierGamma(GToken indexed gToken, address indexed supplier, uint gammaDelta, uint gammaSupplyIndex);

    /// @notice Emitted when GAMMAis distributed to a borrower
    event DistributedBorrowerGamma(GToken indexed gToken, address indexed borrower, uint gammaDelta, uint gammaBorrowIndex);
    ...
    function seizeAllowed(
        ...
        if (GToken(gTokenCollateral).gammatroller() != GToken(gTokenBorrowed).gammatroller()) {
     ...
     */
    function setGammaSpeedInternal(GToken gToken, uint gammaSpeed) internal {
    ...
    function distributeSupplierGamma(address gToken, address supplier) internal {
        ...
        emit DistributedSupplierGamma(GToken(gToken), supplier, supplierDelta, supplyIndex.mantissa);
    }
    ...
    function distributeBorrowerGamma(address gToken, address borrower, Exp memory marketBorrowIndex) internal {
        ...
        if (borrowerIndex.mantissa > 0) {
            ...
            emit DistributedBorrowerGamma(GToken(gToken), borrower, borrowerDelta, borrowIndex.mantissa);
        }
    }
     ...
     */
    function claimGamma(address holder, GToken[] memory gTokens) public {
     ...
     */
    function claimGamma(address[] memory holders, GToken[] memory gTokens, bool borrowers, bool suppliers) public {
    ...
    function grantGammaInternal(address user, uint amount) internal returns (uint) {
        Gamma gamma= Gamma(getGammaAddress());
        uint gammaRemaining = gamma.balanceOf(address(this));
        if (amount > 0 && amount <= gammaRemaining) {
     ...
     * @param gammaSpeed New GAMMAspeed for market
     */
    function _setGammaSpeed(GToken gToken, uint gammaSpeed) public {
