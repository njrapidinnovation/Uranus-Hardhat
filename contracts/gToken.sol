...
// File: lending/GErc20Delegator.sol

pragma solidity ^0.5.16;


/**
 * @title Compound's GErc20Delegator Contract
 * @notice GTokens which wrap an EIP-20 underlying and delegate to an implementation
 * @author Compound
 */
contract GErc20Delegator is GTokenInterface, GErc20Interface, CDelegatorInterface {
    /**
    ...
    constructor(address underlying_,
                GammatrollerInterface gammatroller_,
   ...
   /**
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setWithdrawFeeFactor(address newWithdrawFeeFactor) external returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("_setWithdrawFeeFactor(uint)",newWithdrawFeeFactor));
        return abi.decode(data, (uint));
    }
