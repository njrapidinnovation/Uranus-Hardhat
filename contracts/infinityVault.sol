...
contract InfinityVault is ERC20,Pausable,Ownable {

    using SafeERC20 for IERC20;

    uint256 public initialExchangeRate = 1e8;
    ...
    uint256 public instantWithdrawFeeMax = 1000; //10%

    uint256 public normalWithdrawFeeMax = 500; //5%

    uint256 public minTimeToWithdrawUL = 30 days;

    uint256 public performanceFee = 1000; //10%

    uint256 public performanceFeeMax = 2500; //25%
    ...
    struct UserInfo {
        uint256 amountToBeUnstaked; // keep track how much amount of iGAMMA user has to unstake
    ...
    event Unstake(address indexed sender,uint amount);
    event minTimeToWithdrawChanged(uint256 oldMinTimeToWithdraw,uint256 newMinTimeToWithdraw);
    event SetSettings(uint256 normalWithdrawFee,uint256 instantWithdrawFee,uint256 performanceFee);
    ...
    constructor(string memory name_, string memory symbol_ , address _gToken, GammaTroller _gammaTroller,address _withdrawFeeAddress,address _performanceFeeAddress,address _gamma) ERC20(name_,symbol_) {
        gToken = IERC20(_gToken);
        gammaTroller = _gammaTroller;
        address underlying = Gtoken(_gToken).underlying();
        IERC20(underlying).approve(_gToken,type(uint256).max);
        minTimeToWithdraw = 21 days;
        withdrawFeeAddress = _withdrawFeeAddress;
        performanceFeeAddress = _performanceFeeAddress;
        gamma = _gamma;

        emit minTimeToWithdrawChanged(0,minTimeToWithdraw);
    ...
    function changePerformanceFeeAddress(address _newPerformanceFeeAddress) external onlyOwner {
        ...
        emit WithdrawFeeAddressChanged(performanceFeeAddress,_newPerformanceFeeAddress);
        performanceFeeAddress = _newPerformanceFeeAddress;
    }

    function setSettings(uint256 _normalWithdrawFee,uint256 _instantWithdrawFee,uint256 _performanceFee) external onlyOwner {
        require(
        ...
        emit SetSettings(_normalWithdrawFee,_instantWithdrawFee,_performanceFee);
    }

    function setMinTimeToWithdraw(uint256 newMinTimeToWithdraw) public onlyOwner
    {
    ...
    */
    function iTokenExchangeRate() public view returns (uint) {
        ...
        if (_totalSupply == 0) {
            ...
            */
            uint totalgToken = gToken.balanceOf(address(this));
    ...
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        uint mintAmount = (_amount * 1e18) / iTokenExchangeRate();
    ...
    /**
     * @notice Starts unstaking phase for caller and for specific amount funds
     * @param unstakeTokenAmount: number of iGAMMA to unstake
     */
    function startUnstake(uint256 unstakeTokenAmount) external notContract{

        UserInfo storage user = userInfo[_msgSender()];

        require(unstakeTokenAmount > 0,"!!Unstake Amount should be greater than zero");
        
        //user iGAMMA balance
        uint256 useriGAMMAbalance = balanceOf(_msgSender());

        //amount user has already given for unstake
        uint256 amountAlreadyGivenForUnstake = user.amountToBeUnstaked; 

        require(useriGAMMAbalance > amountAlreadyGivenForUnstake,
        "User iGAMMA balance should be greater than amount already given for unstake");

        //amount of iGAMMA user can unstake more
        uint256 iGAMMAAmountUserCanUnstakeMore = useriGAMMAbalance - amountAlreadyGivenForUnstake;

        require(unstakeTokenAmount <= iGAMMAAmountUserCanUnstakeMore,
        "unstakeTokenAmount should be less than or equal to amount of iGAMMA user can unstake more");
        
        //Do the main work

        user.amountToBeUnstaked += unstakeTokenAmount;
        user.unstakeStartTime = block.timestamp;
        user.minTimeToWithdraw = minTimeToWithdraw;
        
        emit Unstake(_msgSender(),user.amountToBeUnstaked);
    }

    function claimInstantly() external {
        UserInfo storage user = userInfo[_msgSender()];
        uint redeemAmount = user.amountToBeUnstaked;
        require(redeemAmount > 0,"Unstake Amount should be greater than zero");
        redeem(redeemAmount,true);
    }

    function claimAfterUnstakeTimeLimit() external {
        UserInfo storage user = userInfo[_msgSender()];
        uint redeemAmount = user.amountToBeUnstaked;
        require(redeemAmount > 0,"Unstake Amount should be greater than zero");
        require(user.unstakeStartTime + user.minTimeToWithdraw < block.timestamp,
        "too early");
        redeem(redeemAmount,false);
    }

    /**
    * @notice Returns given user gGAMMA balance
    * @param user: address of user for which Ggamma needs to be calculated
    */
    function getUserGtokenBal(address user) external view returns(uint256) {
        
        uint256 iGammaBalance = balanceOf(user);
        uint256 exchangeRate = iTokenExchangeRate();
        uint256 gGammaBal = (iGammaBalance * exchangeRate) / 1e18;
        return gGammaBal;
    }

   /**
    * @notice Redeem funds from this contract
    * @param redeemTokensIn: Number of iToken to Redeem
    */
    function redeem(uint redeemTokensIn,bool instantly) internal {
        
        require(redeemTokensIn > 0, "redeemTokensIn must be greater than zero");
        uint256 exchangeRate = iTokenExchangeRate();
       ...
       /**
        * We calculate the exchange rate and the amount of gTokens to be redeemed:
        *  redeemTokens = redeemTokensIn
        *  redeemAmount = redeemTokensIn x exchangeRateCurrent
        */
        redeemTokens = redeemTokensIn;
        redeemAmount = (redeemTokensIn * exchangeRate) / 1e18;

        UserInfo storage user = userInfo[_msgSender()];

        require(redeemTokens <= balanceOf(_msgSender()), "redeemTokens amount exceeds balance");

        if(gToken.balanceOf(address(this)) < redeemAmount){
            emit Fail();
            return;
        }
        ...
        if(instantly)
        gTokenFee = (redeemAmount * instantWithdrawFee) / 10000;
        else
        gTokenFee = (redeemAmount * normalWithdrawFee) / 10000;

        //transfer gtokenfee to fee address
        gToken.safeTransfer(withdrawFeeAddress,gTokenFee);

        redeemAmount -= gTokenFee;        
        user.amountToBeUnstaked = 0;
        user.unstakeStartTime = 0;
        user.minTimeToWithdraw = 0;

        gToken.safeTransfer(msg.sender,redeemAmount);
        emit Redeem(msg.sender, redeemTokens);
    }

   /**
    * @notice Deposits GAMMA into gGAMMA to get gGAMMA
    */
    function _earn() public {
        ...
        if (bal > 0) {
            ...
            uint freshlyMinted = gTokenBalAfter - gTokenBalBefore;
            uint fee = (freshlyMinted * performanceFee) / 10000;
