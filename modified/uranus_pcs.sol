...
abstract contract StratX2 is Ownable, ReentrancyGuard, Pausable {
    ...
    bool public isAutoComp; // this vault is purely for staking. eg. WBNB-GAMMA staking vault.
    ...
    uint256 public buyBackRate = 1000; // 1000;  10%(buy-back and burn fee)
    ...
    uint256 public stratRewardPerShare = 0;

    address[] public earnedToGAMMAPath;
    ...
    function userPendingGammaProfit(address userAddress) public view virtual returns (uint256 pendingProfitInCake,uint256 pendingProfitInGamma){
        ...
        if(stratRewardPerShare > 0 && user.shares > 0){
            uint cakeBal = IPancakeswapFarm(farmContractAddress).pendingCake(pid,address(this));
...
contract GammaStrategy_AQUA is StratX2 {
    ...
    constructor(
        ...
        bool _isAutoComp,
        address[] memory _earnedToGAMMAPath,
        ...
        earnedToGAMMAPath = _earnedToGAMMAPath;
        ...
        planetRouterAddress = _addresses[12];
        entranceFeeFactor = _entranceFeeFactor;
        withdrawFeeFactor = _withdrawFeeFactor;
